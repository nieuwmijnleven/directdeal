 ...
 *
 * <p>This entity stores account information including email, password hash,
 * name, activation status, and associated authorities.</p>
 *
 * <p>The password field stores the hashed password only and is ignored
 * during JSON serialization for security reasons.</p>
 *
...
public class Account extends AbstractAuditingEntity {
    ...
    /**
     * Updates mutable fields from AccountDTO.
     *
     * @param dto Data transfer object containing updated fields
     */
    public void updateFrom(AccountDTO dto) {
        // Email update is intentionally commented out to prevent email changes
        // if (Objects.nonNull(dto.getEmail())) this.email = dto.getEmail();
        if (Objects.nonNull(dto.getName())) this.name = dto.getName();
    }

    /**
     * Changes the password hash to the newly encoded password.
     *
     * @param encodedNewPassword encoded new password string
     */
